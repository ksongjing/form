...
class GameList extends Component {
    ...
    render() {
        const { curLanguage, gameList, totalItems } = this.props;
        ...
        return (
            <div className="gamelist">
                ...
                <div className="container gamelist-empty" style={{display: gameList && gameList.length > 0 ? 'none' : 'flex'}}>No data</div>
                <GamePanel isEmpty={gameList && gameList.length > 0 ? false : true} items={gameList} curLanguage={curLanguage} />
                <div className="gamelist-loadmore" onClick={this.loadMore}
                     style={{display: this.state.isSearchAction || (curItems + PAGE_NUM >= totalItems) ? 'none' : 'flex'}}>
                    加载更多
                </div>
